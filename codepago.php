...
if (isset($_POST['update'])) {
    ...
    Openpay::setProductionMode(
        filter_var($_ENV['OPENPAY_PRODCUTION_MODE'], FILTER_VALIDATE_BOOLEAN)
    );
    ...
    try {
        ...
        if ($method === 'bank_account') {
            ...
            if ($charge->status == 'completed') {
                ...
                // LLAMADA AL CORREO DE CONFIRMACIÓN DE TARJETA
                enviarCorreoConfirmacion($email, $nombre, $identificador, $montoFinal);
...
function enviarCorreoConfirmacion($emailCliente, $nombreCliente, $order_id, $total) {
    $mail = new PHPMailer(true);

    try {
        // Configuración SMTP (Asegúrate de usar los mismos datos reales que usas en notifyCustomer)
        $mail->isSMTP();
        $mail->Host       = 'mail.dominio.mx'; // O tu servidor SMTP real
        $mail->Port       = 465;                // O el puerto correcto (ej. 587 o 465)
        $mail->SMTPAuth   = true;
        $mail->Username   = 'no-reply@dominio.mx';
        $mail->Password   = '=@dH6mqA5H7%MEa,';
        $mail->SMTPSecure = 'ssl';               // O PHPMailer::ENCRYPTION_STARTTLS si usas 587
        ...
        $mail->CharSet = 'UTF-8';
        $mail->isHTML(true);
        $mail->Subject = 'Confirmación de tu compra #' . $order_id;
        ...
        $mail->Body    = "
            ...
            <p>Gracias por tu compra. Pronto nos pondremos en contacto contigo para el envío.</p>
        ";
        
        $mail->AltBody = "Hola {$nombreCliente}, gracias por tu compra. Tu pedido #{$order_id} por un total de $" . number_format($total, 2) . " MXN ha sido procesado con éxito.";
