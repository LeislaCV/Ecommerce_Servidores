...
// --- Consultas seguras con validación ---

// Obtenemos Envío por ID=1
$consultaEnvio = $con->query("SELECT valoruno, valordos FROM configuraciones WHERE id=1 LIMIT 1");
$env = $consultaEnvio->fetch_assoc();

// Si $env es nulo, asignamos 0 para evitar el error de acceso a offset
$envioMinimo = $env ? $env['valoruno'] : 0;
$envioCosto = $env ? $env['valordos'] : 0;

// Comisión
$consultaComision = $con->query("SELECT valoruno FROM configuraciones WHERE id=4 LIMIT 1");
$com = $consultaComision->fetch_assoc();

$comisionValor = $com ? str_replace('%', '', $com['valoruno']) : '0';
